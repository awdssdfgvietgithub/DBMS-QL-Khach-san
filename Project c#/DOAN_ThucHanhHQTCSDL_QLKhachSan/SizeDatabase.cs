...
namespace DOAN_ThucHanhHQTCSDL_QLKhachSan
{
    public partial class SizeDatabase : Form
    {
        ...
        public void load_database_gridview1()
        {
            try
            {
                SqlCommand c = new SqlCommand("SELECT name, state_desc, (size * 8) / 1024 'Size (MB)', (max_size * 8) / 1024 'Maxsize (MB)', growth '(%)' FROM sys.database_files;", conn);
                SqlDataAdapter da = new SqlDataAdapter(c);
                DataTable dt = new DataTable();
                da.Fill(dt);
        ...
        private void btn_link_Click(object sender, EventArgs e)
        {
            ...
            {
                txt_filename.Text = F.SelectedPath;
            }
        }

        private void btn_upmax_Click(object sender, EventArgs e)
        {
            int b = int.Parse(txt_maxs.Text);
            string tenfile = txt_tenfile.Text;
            try
            {
            //------------------------------------------------------//
            ...
            if (txt_maxs.Text == "" && txt_tenfile.Text == "")
            {
                MessageBox.Show("Bạn Chưa Nhập Thông Tin Để tăng size");
            }
            else
            {
                SqlCommand c = new SqlCommand("Alter database QL_KS_TUAN_1 Modify file (name ='" + tenfile + "',maxsize = " + b + " MB)", conn);
                c.ExecuteNonQuery();
                SqlDataAdapter da = new SqlDataAdapter(c);
                MessageBox.Show("Up maxsize Thành Công ");
                load_database_gridview();
                load_database_gridview1();
            }
            if (conn.State == ConnectionState.Open)
